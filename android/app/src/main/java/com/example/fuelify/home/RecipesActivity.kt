...
class RecipesActivity : AppCompatActivity() {
    ...
    private fun bindMeals(meals: List<SearchMealItem>, container: LinearLayout) {
        meals.forEach { meal ->
            ...
            updateSaveButton(saveBtn, SavedRecipesManager.isSaved(this, meal.mealId))

            saveBtn.setOnClickListener {
                val nowSaved = SavedRecipesManager.isSaved(this, meal.mealId)
                if (nowSaved) {
                    ...
                    SavedRecipesManager.save(this, meal)
                    updateSaveButton(saveBtn, true)
                    Toast.makeText(this, "Saved! View in Saved Recipes", Toast.LENGTH_SHORT).show()
                }
            }

            // Switch button — always visible, opens bottom sheet
            val switchBtn = card.findViewById<TextView>(R.id.btnSwitchMeal)
            switchBtn.visibility = View.VISIBLE
            switchBtn.setOnClickListener { showSwitchSheet(meal) }
    ...
    /** Toggles the save button label and tint between saved / unsaved states. */
    private fun updateSaveButton(btn: TextView, saved: Boolean) {
        if (saved) {
            btn.text = "🔖 Saved"
            btn.setTextColor(0xFF888888.toInt())   // blue when saved
        } else {
            btn.text = "🔖 Save"
            btn.setTextColor(0xFF888888.toInt())   // grey when unsaved
        }
    }
    ...
    private fun showSwitchSheet(newMeal: SearchMealItem) {
        ...
        dialog.findViewById<TextView>(R.id.tvSwitchTitle).text    = "Switch with which meal?"
